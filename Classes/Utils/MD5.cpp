...
MD5::MD5()
{
    ...
    _currentDigest[0] = 0x67452301;
    _currentDigest[1] = 0xefcdab89;
    _currentDigest[2] = 0x98badcfe;
    _currentDigest[3] = 0x10325476;
}

void MD5::_hashCore(const char* input, int length)
{
    ...
    if (length >= firstpart)
    {
        ...
        {
            _transform((unsigned char*)input + i);
        }
...
char* MD5::_hashFinal()
{
    ...
    _hashCore((char*)padding, padLen);
    
    // Append length (before padding)
    _hashCore((char*)bits, 8);
    
    // Store state in digest
    _encode(_digest, _currentDigest, 16);
...
void MD5::_decode(unsigned int output[], const unsigned char input[], int len)
{
...
void MD5::_encode(unsigned char output[], const unsigned int input[], int len)
{
...
void MD5::_transform(const unsigned char block[64])
{
    unsigned int a = _currentDigest[0];
    unsigned int b = _currentDigest[1];
    unsigned int c = _currentDigest[2];
    unsigned int d = _currentDigest[3];
    unsigned int x[16];
    _decode(x, block, 64);
...
inline unsigned int MD5::rotateLeft(unsigned int x, int n)
{
    return (x << n) | (x >> (32 - n));
}
